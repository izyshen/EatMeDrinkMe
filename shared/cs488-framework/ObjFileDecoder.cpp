...
void ObjFileDecoder::decode(
    ...
    vec3 temp_maxCoordinate, temp_minCoordinate;
    bool firstPass = true;
    ...
    while (!in.eof()) {
	    ...
	    if (currentLine.substr(0, 2) == "o ") {
            ...
            s >> vertex.y;
            s >> vertex.z;
            if ( firstPass ) {                  // record first vertex as initalizer
                temp_maxCoordinate = vertex;
                temp_minCoordinate = vertex;
                firstPass = false;
            } else {

                temp_maxCoordinate.x = vertex.x > temp_maxCoordinate.x ? vertex.x : temp_maxCoordinate.x;
                temp_maxCoordinate.y = vertex.y > temp_maxCoordinate.y ? vertex.y : temp_maxCoordinate.y;
                temp_maxCoordinate.z = vertex.z > temp_maxCoordinate.z ? vertex.z : temp_maxCoordinate.z;

                temp_minCoordinate.x = vertex.x < temp_minCoordinate.x ? vertex.x : temp_minCoordinate.x;
                temp_minCoordinate.y = vertex.y < temp_minCoordinate.y ? vertex.y : temp_minCoordinate.y;
                temp_minCoordinate.z = vertex.z < temp_minCoordinate.z ? vertex.z : temp_minCoordinate.z;
    ...
    in.close();
    maxCoordinate = temp_maxCoordinate;        // save coordinates
    minCoordinate = temp_minCoordinate;
