...
void TutorialExample::uploadVertexDataToVbo()
{
	...
	// Generate a vertex buffer object to hold the triangle's vertex data. (gives pointer that the library knows what to do with; you don't need to worry about it)
	// places a value in m_vbo
	...
	glBufferData(GL_ARRAY_BUFFER,
                 ...
                 triangleVertices,
                 GL_STATIC_DRAW // never going to change this data
    );

    // in A1, we want to store a static cube. Will pass GL_STATIC_DRAW to it


	// Unbind the target GL_ARRAY_BUFFER, now that we are finished using it.
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	// call this to get errors from graphics card
	CHECK_GL_ERRORS;
}
...
{
	// Generate and bind the VAO(some number) that will store the data mapping.
	glGenVertexArrays(1, &m_vao);
	glBindVertexArray(m_vao); // bind buffer object (as long as you only bind 1 vertex arr to one buffer, it stays bound)
	...
	// Map vertex positions from m_vbo into vertex attribute slot
	{
        ...
        // size of data is 3 floats
        // GL_FALSE: don't normalize data passed 
...
void TutorialExample::appLogic()
{
	...
	// glBufferSubData allows you to pass just a portion of the data
}
